...
//读取文件
int
PointCloudOperation::readData(const std::string& file_path){
...
//点云归一化
int
PointCloudOperation::getNormalizedPointCloud(){
    if(_point_cloud_ptr == nullptr){
...
//    std::cerr<<min_x_y_z.x<<" "<<min_x_y_z.y<<" "<<min_x_y_z.z<<std::endl;
//    std::cerr<<max_x_y_z.x<<" "<<max_x_y_z.y<<" "<<max_x_y_z.z<<std::endl;
    //包围盒中心
    PointT center;
    ...
    //pcl::getMinMax3D(*_point_cloud_ptr,min_x_y_z,max_x_y_z);
    //std::cout<<min_x_y_z.x<<" "<<min_x_y_z.y<<" "<<min_x_y_z.z<<std::endl;
    //std::cout<<max_x_y_z.x<<" "<<max_x_y_z.y<<" "<<max_x_y_z.z<<std::endl;
    return 1;
}
